...
L :	system("clear");
	...
	int N, l = 0;
	float J = 1.0, h = 1.0, min_ev, concurrence;
	string filename;
...
// Concurrence	
	...
	filename = "sp_con" + to_string(n) + ".dat";
	...
	for (h = 0.3; h <= 3; h = h + 0.05)								// Running h from 0.1 to 3 with intervals of 0.05
	{
		...
		fspcon<<J/h<<"   "<<concurrence<<endl;						// Store values in the file
...
// Dcon
   	...
   	filename = "sp_dcon" + to_string(n) + ".dat";
	fspcon.open(filename, ios::trunc);
	//cout<<"\n Filename : "<<filename;
	
	for(i = 1; i < conc.size(); i++)									// 59 elements in conc which will yield 58 averages. But the index goes from 0 to 58
	{
		float derivative = (conc[i] - conc[i-1]) / (lambda[i] - lambda[i-1]);
		dconc.push_back(derivative);
	}
	 
	i = 0; 
	for(h = 0.3; h < 2.95; h = h + 0.05)								
		fspcon << (J/h + J/(h + 0.05))/2 << "   "<<dconc[i++]<<endl;			// Taking average of lambda because we calculated the derivative at the midpoint
...
// p 'sp_con4.dat' w l, 'sp_con6.dat' w l, 'sp_con8.dat' w l, 'sp_con10.dat' w l, 'sp_con12.dat' w l, 'sp_con14.dat' w l
// p 'sp_dcon4.dat' w l, 'sp_dcon6.dat' w l, 'sp_dcon8.dat' w l, 'sp_dcon10.dat' w l, 'sp_dcon12.dat' w l, 'sp_dcon14.dat' w l
